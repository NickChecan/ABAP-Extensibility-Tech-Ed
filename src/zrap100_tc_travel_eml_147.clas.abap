CLASS zrap100_tc_travel_eml_147 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS zrap100_tc_travel_eml_147 IMPLEMENTATION.

  METHOD class_setup.
    " create the test doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZRAP100_R_TravelTP_147' ) ) ).
...
*            FROM VALUE #( ( %cid_ref = 'ROOT1' ) )
    ...
    SELECT * FROM ZRAP100_R_TravelTP_147 INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
